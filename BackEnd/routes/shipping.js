...
const AddressCache = require('../models/adressCache');
...
router.post('/validate-address', async (req, res) => {
  ...
  try {
    ...
    if (!q) {
      return res.status(400).json({ result: false, error: 'Missing address (q or line1/city/postcode)' });
    }
    if (!apiKey) {
      return res.status(500).json({ result: false, error: 'Missing OKAPI_CONTROL_KEY' });
    }
    ...
    if (arr.length > 1) {
      const suggestions = arr.slice(0, 10).map(x => ({ adresse: x.adresse, code: x.code }));
      await AddressCache.create({
        addressHash,
        suggestions,
        source: 'okapi',
    ...
    console.error('[ADDR] validate error:', err?.response?.data || err.message);
    const msg = err?.response?.data?.message || err.message;
    return res.status(500).json({ result: false, error: msg });
  }
});
